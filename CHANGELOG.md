...
- `ReplayManager` data layer: `get_sessions` fetches Race sessions from F1 livetiming Index.json, `load_session` now downloads all 8 timing streams in parallel (TrackStatus, RaceControlMessages, SessionStatus, WeatherData, TimingData, TimingAppData, DriverList, LapCount), builds a unified chronological event list anchored to lights-out, and replays every event through the same `F1Listener._handle_feed` handler that live mode uses — so weather, driver positions, session info, and lap counts all update identically to a live session
...
- Fixed: stopping replay (via Stop button or natural end of playback) now resets the LED to idle and clears the track status to "unknown" — previously the LED stayed on the last replay flag state after stopping
